...
var remplirTableauVoteVainqueurs = function (autresVotesObj){
	...
	Object.keys(autresVotesObj).forEach(function(key) {
		var pseudo = key;
    		...
    		if(i%2 == 0){
    			str += '<tr class="success"><td>'+pseudo+'</td><td>'+vote1+'</td><td>'+vote2+'</td><td>'+vote3+'</td></tr>'
    		}else{
...
var remplirSaLigneVoteVainqueur = function(pseudo, pays1, pays2, pays3){
	...
	if(pays2)
		document.getElementById('ligneMesVotesVainqueursEuro').innerHTML += "<td>"+arrPaysEuro[parseInt(pays2)]+"</td>";
	if(pays3)
		document.getElementById('ligneMesVotesVainqueursEuro').innerHTML += "<td>"+arrPaysEuro[parseInt(pays3)]+"</td>";
};
